...
/*
 * Input: Node np - a node of the avl         // int values[]  = {3, 2, 1, 5, 4};
tree.
...
{
        Node *temp_root_left = np->left;
        Node *temp_root_left_right = np->left->right;
        np->left->right = np;
        np->left = temp_root_left_right;

        np->height = 1 + max(nodeHeight(np->left), nodeHeight(np->right));
        temp_root_left->height = 1 +
        max(nodeHeight(temp_root_left->left),
            nodeHeight(temp_root_left->right));

        return temp_root_left;
}
...
{
        Node *temp_root_right = np->right;
        Node *temp_root_right_left = np->right->left;
        np->right->left = np;
        np->right = temp_root_right_left;

        np->height = 1 + max(nodeHeight(np->left), nodeHeight(np->right));
        temp_root_right->height = 1 +
        max(nodeHeight(temp_root_right->left),
            nodeHeight(temp_root_right->right));

        return temp_root_right;
}
